...
namespace gum{
    ...
    /**
     * @brief Adds the ancestors of ``x`` in the Bayesian network ``bn`` to the set ``P`` 
     * 
     * @param x 
     * @param dm 
     * @param anc 
     */
    void ancestor(NodeId x, const DirectedModel<GUM_SCALAR>& dm, const NodeSet& anc){
        for(const auto& p : dm.parents(x)){
            if(anc.contains(p)) continue;
            anc.insert(p);
            ancestor(p, dm, anc);
        }
    }

    // def ancestor(x: NodeId, dm: DirectedModel, anc: NodeSet):
    //   """
